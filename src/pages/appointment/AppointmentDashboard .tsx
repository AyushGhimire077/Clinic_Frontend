...
const AppointmentDashboard = () => {
  const [activeTab, setActiveTab] = useState<"appointments" | "pending">(
    "appointments"
  );
  ...
  return (
    <div className="min-h-screen bg-background py-8 px-4">
      <div className="max-w-4xl mx-auto">
            ...
            ? currentTab?.cards.map((card) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    <div
                      className={`w-12 h-12 rounded-lg bg-linear-to-br ${card.color} flex items-center justify-center shadow-soft`}
                    >
                      {card.icon}
                    </div>
        ...
        {/* OVERVIEW BOXES */}
        {currentTab?.id == "appointments" && (
          <div className="mt-10 grid grid-cols-1 sm:grid-cols-4 gap-4">
            {todayStats.map((stat, idx) => (
              <div
                key={idx}
                className="p-4 bg-surface border border-border rounded-xl shadow-soft"
              >
